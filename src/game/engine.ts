/* ============================================================
 * NFLMatchEngine — simulação pura, sem UI.
 * Lógica: DOWNS & YARDS, drive a drive, com eventos ao vivo,
 * momentum, clutch rating, playbooks, rivalidades e box score rico.
 * ============================================================ */
...
export class NFLMatchEngine {
  ...
  private pens = { casa: 0, fora: 0 };
  private penYds = { casa: 0, fora: 0 };
  ...
  private adjustMomentum(side: 'casa' | 'fora', delta: number, result: string) {
    ...
    this.momentum[other] = clamp(this.momentum[other] - delta * 0.4, 0, 100);
    this.lastMomentumResult = result;
  }

  private momentumMultiplier(off: Unit): number {
    const m = off === this.uc ? this.momentum.casa : this.momentum.fora;
    return 0.9 + (m / 100) * 0.25; // 0.90 .. 1.15
  }
  ...
  private drive(off: Unit, def: Unit): DriveOutcome {
    ...
    if (off.qb && off.qb.lesao > 0) {
      ...
      this.emit({ kind: 'qb', texto: `${t.sigla}: ${shortName(off.qb.nome)} lesionado — entra ${sub ? shortName(sub.nome) : '—'}.`, clock: this.clock });
      off.qb = sub;
    }
    ...
    while (true) {
      ...
      if (down === 4) {
        ...
        if (toGo <= goLimit && (aggr >= 45 || toGo <= 1)) {
          ...
          if (desc.texto) { segTextos.push(desc.texto); if (desc.tipo !== 'ok') segTipo = desc.tipo; }
          if (r.turnover) {
            out.tos++;
            ...
            this.emit({ kind: 'turnover', texto: segTextos.join('\n'), tipo: 'turn', posse: side === 'casa' ? 'fora' : 'casa', ball: 100 - ball, down: 1, toGo: 10, clock: this.clock, runYds: segRun, passYds: segPass, penalties: segPen });
      ...
      if (ball >= 80 && !this.rzEntered(side)) this.markRz(side);
      const r = this.scrimmage(off, def, ball, down, toGo);
      ...
      if (r.faltaTeam) {
        segPen++;
        this.penalize(r.faltaTeam === 'off' ? side : (side === 'casa' ? 'fora' : 'casa'), 10);
      }

      if (r.turnover) {
        out.tos++;
        if (desc.texto) segTextos.push(desc.texto);
        this.emit({ kind: 'turnover', texto: segTextos.join('\n') || 'Turnover!', tipo: 'turn', posse: side === 'casa' ? 'fora' : 'casa', ball: 100 - ball, down: 1, toGo: 10, clock: this.clock, runYds: segRun, passYds: segPass, penalties: segPen });
        segTextos = []; segRun = 0; segPass = 0; segPen = 0;
        break;
      }
      if (r.fgMade) { out.pts = 3; if (desc.texto) { segTextos.push(desc.texto); segTipo = 'score'; } flush(); break; }

      if (desc.texto) segTextos.push(desc.texto);
      if (desc.tipo === 'big' || desc.tipo === 'pen' || desc.tipo === 'inj') segTipo = desc.tipo;
      ...
      if (wasThird) {
        this.third[side].att++;
        if (firstDown || ball >= 100) this.third[side].conv++;
      }

      const relevante = firstDown || r.yds >= 8 || r.yds <= -3 || r.faltaTeam !== undefined ||
        (r.gain !== null && r.gain.yds >= 12) || ball >= 75 || down === 3 || down === 4 ||
  ...
  private rzFlags = { casa: false, fora: false };
  private rzEntered(side: 'casa' | 'fora') { return this.rzFlags[side]; }
  private markRz(side: 'casa' | 'fora') { this.rzFlags[side] = true; this.rz[side].att++; }
  private resetRz() { this.rzFlags = { casa: false, fora: false }; }
  ...
  private scrimmage(off: Unit, def: Unit, ball: number, down: number, toGo: number): ScrimRes {
    ...
    if (nerv > 0.32 && this.rng.chance(nerv * 0.03)) {
      ...
      res.faltaTeam = 'off';
      this.penalize(off === this.uc ? 'casa' : 'fora', 5);
      return res;
    }
  ...
  private playRun(off: Unit, def: Unit, dn: string, spot: string, res: ScrimRes, nerv: number) {
    ...
    const clutch = this.clutchFactor(rb);
    let yds = 3.4 + qn * 2.4 + this.rng.gauss(0, 2.5);
    const big = this.rng.chance(clamp(0.05 + (rb.attrs.velocidade - def.coverage) * 0.0016 + qn * 0.02, 0.02, 0.13));
    if (big) yds = 15 + this.rng.int(0, 30);
    yds = clamp(Math.round(yds * clutch * this.momentumMultiplier(off)), -4, 45);
    ...
    if (this.rng.chance(fumP)) {
      ...
      if (rec) { this.addStat(rec.id, 'tackles', 1); this.line(rec).tackles = (this.line(rec).tackles ?? 0) + 1; }
      this.adjustMomentum(off === this.uc ? 'casa' : 'fora', -18, 'fumble');
      this.adjustMomentum(def === this.uc ? 'casa' : 'fora', 18, 'fumble');
      this.maybeInjury(off, def);
      return;
    }

    const tackler = this.rng.pick([...def.lb, ...def.dl, ...def.s]);
    if (tackler && yds < 8) { this.addStat(tackler.id, 'tackles', 1); this.line(tackler).tackles = (this.line(tackler).tackles ?? 0) + 1; }

    if (yds >= 12) this.log(`${dn}, ${spot} — ${shortName(rb.nome)} rompe o tackle e DISPARA ${yds} jardas!`, 'big');
    else if (yds <= 0) this.log(`${dn}, ${spot} — ${shortName(rb.nome)} é parado no backfield (${yds} jd).`, 'ok');
    ...
    rl.longRush = Math.max(rl.longRush ?? 0, yds);
    if (yds >= 25) this.considerBest(yds, `Corrida de ${yds} jardas de ${shortName(rb.nome)}`, off.team.id, false);
  ...
  private playPass(off: Unit, def: Unit, dn: string, spot: string, res: ScrimRes, nerv: number) {
    ...
    const complP = clamp(0.62 + qn * 0.08 - pressure * 0.03 + this.clima.pass * 0.004 - nerv * 0.06, 0.35, 0.78);
    const sackP = clamp(0.055 + pressure * 0.022 - qb.attrs.velocidade * 0.0004, 0.02, 0.13);
    const intP = clamp(0.02 + pressure * 0.006 - qn * 0.006 + nerv * 0.010, 0.006, 0.10);
    const clutch = this.clutchFactor(qb);
    ...
    if (this.rng.chance(sackP)) {
      ...
      this.adjustMomentum(off === this.uc ? 'casa' : 'fora', -8, 'sack');
    ...
    ql.cmp = (ql.cmp ?? 0) + 1;
    ql.py = (ql.py ?? 0) + yds;
    ql.longPass = Math.max(ql.longPass ?? 0, yds);
    const al = this.line(alvo);
    al.rec = (al.rec ?? 0) + 1;
    al.recYds = (al.recYds ?? 0) + yds;
    al.longRec = Math.max(al.longRec ?? 0, yds);
    if (yds >= 25) this.considerBest(yds, `Passe de ${yds} jardas de ${shortName(qb.nome)} para ${shortName(alvo.nome)}`, off.team.id, false);
    res.passY += yds;
  ...
  private clutchFactor(p: Player): number {
    const situacao = this.scoreC === this.scoreF && this.clock > 2700;
    if (!situacao) return 1;
    return 0.9 + (p.clutchRating / 100) * 0.25;
  }
  ...
  private touchdown(off: Unit, gain: ScrimRes['gain']): number {
    ...
    this.adjustMomentum(side, 15, 'td');
    if (this.rzFlags[side]) { this.rz[side].td++; this.resetRz(); }
    if (gain && gain.tipo === 'pass' && off.qb) {
      ...
      this.line(off.qb).ptd = (this.line(off.qb).ptd ?? 0) + 1;
      const gl = this.line(gain.p);
      gl.recTD = (gl.recTD ?? 0) + 1;
      this.considerBest(gain.yds, `Passe de ${gain.yds} jardas de ${shortName(off.qb.nome)} para ${shortName(gain.p.nome)} — TOUCHDOWN`, t.id, true);
    } else {
      const rb = gain?.p ?? off.rbs[0] ?? off.qb;
      this.log(`TOUCHDOWN do ${t.sigla}! ${rb ? shortName(rb.nome) : t.sigla} invade a end zone.`, 'score');
      if (rb) {
        this.addStat(rb.id, 'rtd', 1);
        this.line(rb).rtd = (this.line(rb).rtd ?? 0) + 1;
  ...
  private maybeInjury(off: Unit, def: Unit, baseP = 0.0045) {
    ...
    let pool: Player[];
    if (offense) {
      pool = [...(off.qb ? [off.qb] : []), ...off.rbs.slice(0, 2), ...off.wrs.slice(0, 3), ...(off.te ? [off.te] : []), ...off.ol.slice(0, 5)];
    } else {
  ...
  private since(before: number): { texto: string; tipo: LineTipo } {
    ...
    return { texto: ls.map(l => l.t).join('  •  '), tipo: ls.some(l => l.tipo === 'pen') ? 'pen' : ls[0].tipo };
  }
  ...
  private passerRating(cmp: number, att: number, yds: number, td: number, int: number): number {
    if (att <= 0) return 0;
    const c = (x: number) => clamp(x, 0, 2.375);
    const a = c((cmp / att - 0.3) * 5);
    const b = c((yds / att - 3) * 0.25);
    const d = c((td / att) * 20);
    const e = c(2.375 - (int / att) * 25);
  ...
  private buildBox(qC: number[], qF: number[], tot: { yC: number; yF: number; rC: number; rF: number; pC: number; pF: number; toC: number; toF: number; sC: number; sF: number }) {
    ...
    return {
      ...
      faltas: { casa: this.pens.casa, fora: this.pens.fora },
      leaders: [
