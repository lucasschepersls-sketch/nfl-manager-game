...
export interface Side { team: Team; players: Player[]; staff: Staff[]; }

interface ScrimRes {
  yds: number; ball: number; secs: number; turnover: boolean; td: boolean;
  fgMade: boolean; passY: number; rushY: number; faltaTeam?: 'off' | 'def';
...
export class NFLMatchEngine {
  ...
  private applyMods(u: Unit, side: Side, isCasa: boolean) {
    ...
    // moral suavizada (±1.4) — não pode transformar o jogo em elástico
    const moral = (side.team.moral - 55) / 25;
    // "12º homem" balanceado: +1.0 a +2.2 de rating (≈ +3 pts reais), cresce com a pressão
    const home = isCasa && !this.opts?.neutro ? clamp(1.0 + side.team.hostilidade / 85, 1.0, 2.2) : 0;
  ...
  /**
   * Nervosismo (Away Game Pressure) — balanceado.
  ...
  private nervousness(off: Unit): number {
    ...
    let nerv = 0.10 + this.casa.team.hostilidade / 400;
    if (!veteranoImune) {
      if (qb.jogosCarreira < 10) nerv += 0.12;   // inexperiente
      if (qb.ovr < 70) nerv += 0.08;             // rating baixo
      if (off.qbBackup) nerv += 0.20;            // reserva em campo
    }
    ...
    while (true) {
      ...
      if (down === 4) {
        ...
        if (toGo <= goLimit && (aggr >= 45 || toGo <= 1)) {
          ...
          if (r.faltaTeam) { segPen++; this.penalize(r.faltaTeam === 'off' ? side : (side === 'casa' ? 'fora' : 'casa'), 10); }
          ...
          if (r.turnover) {
            ...
            this.emit({
              ...
              posse: side === 'casa' ? 'fora' : 'casa', ball: 100 - ball, down: 1, toGo: 10, clock: this.clock,
              runYds: segRun, passYds: segPass, penalties: segPen,
            });
      ...
      if (r.faltaTeam) { segPen++; this.penalize(r.faltaTeam === 'off' ? side : (side === 'casa' ? 'fora' : 'casa'), Math.abs(r.yds) >= 15 ? 15 : 10); }
  ...
  private scrimmage(off: Unit, def: Unit, ball: number, down: number, toGo: number): ScrimRes {
    ...
    // chamada: tática + situação (~56% passe, como na NFL)
  ...
  private playRun(off: Unit, def: Unit, dn: string, spot: string, res: ScrimRes, nerv: number) {
    ...
    // distribuição realista (~4.3 jd/corrida): muitas jogadas curtas, algumas longas
    let yds = this.rng.weighted([-2, 0, 1, 2, 3, 4, 5, 7, 9, 12], [6, 10, 14, 16, 15, 12, 9, 7, 5, 3]);
    yds += Math.round(qn * 2.2 + this.rng.gauss(0, 1.4));
    ...
    // fumble (nervosismo e fadiga aumentam o risco)
