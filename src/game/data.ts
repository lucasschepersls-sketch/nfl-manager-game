...
export interface TeamDef {
  ...
  forca: number;   // 1-5 (temporada 2025)
  camp: number;    // aproveitamento aproximado em 2025
  estadio: string;
}
export const DIV_NAMES = ['Leste', 'Norte', 'Sul', 'Oeste'];
export const CONF_LABEL: Record<Conf, string> = { AFC: 'AFC', NFC: 'NFC' };
...
export function salaryFor(ovr: number, rng: Rng): number {
  ...
  return Math.round(base * rng.f(0.86, 1.14) * 10) / 10;
}
